...
function testfailover {
    ...
    # Test failover for machines listed in a CSV file.
    ...
    foreach ($row in $csvRows) {
        ...
        if ($row.PERFORM_TESTFAILOVER -ne "yes") {
            Write-Host "`n--- Skipping machine (PERFORM_TESTFAILOVER != yes): $machineName ---" -ForegroundColor DarkGray
            continue
        }

        $testNetworkId  = if ($row.TESTFAILOVER_VNET_ARM_ID) { $row.TESTFAILOVER_VNET_ARM_ID } else { $row.TARGET_VNET_ARM_ID }

        Write-Host "`n--- Starting Test Failover for machine: $($row.SOURCE_MACHINE_ARM_ID) ---" -ForegroundColor Yellow
        ...
        Write-Host "Test Failover Payload:`n$body"

        $res = Invoke-AzRestMethod -Method POST -Path $uri -Payload $body
        Write-Host "Test Failover initiated for $($row.TARGET_MACHINE_NAME). Status code: $($res.StatusCode)." -ForegroundColor Green
        $res

        if ($res.StatusCode -in 200, 201, 202) {
            $row | Add-Member -NotePropertyName "ASR_TESTFAILOVER_STATUS" -NotePropertyValue "test failover started" -Force
        }
        else {
            $row | Add-Member -NotePropertyName "ASR_TESTFAILOVER_STATUS" -NotePropertyValue "test failover failed" -Force
            Write-Host "Test Failover failed for $($row.TARGET_MACHINE_NAME). Response:`n$($res.Content)" -ForegroundColor Red
        }
    }
