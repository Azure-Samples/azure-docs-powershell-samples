#Azure Migrate/ASR - Planned Failover (Migration) Script
#Extracted from Start-ASRExamples.ps1
#requires -Version 7 -module Az.RecoveryServices, Az.Accounts

param (
    [Parameter(Mandatory=$true)][string]$CsvPath             # Path to CSV file for batch planned failover.
)
...
function plannedfailover {
    ...
    # Planned failover (migration) for machines listed in a CSV file.
    ...
    foreach ($row in $csvRows) {
        ...
        if ($row.PERFORM_MIGRATION -ne "yes") {
            Write-Host "`n--- Skipping machine (PERFORM_MIGRATION != yes): $machineName ---" -ForegroundColor DarkGray
            continue
        }

        Write-Host "`n--- Starting Planned Failover (Migration) for machine: $($row.SOURCE_MACHINE_ARM_ID) ---" -ForegroundColor Yellow
        ...
        Write-Host "Planned Failover Payload:`n$body"

        $res = Invoke-AzRestMethod -Method POST -Path $uri -Payload $body
        Write-Host "Planned Failover initiated for $($row.TARGET_MACHINE_NAME). Status code: $($res.StatusCode)." -ForegroundColor Green
        $res

        if ($res.StatusCode -in 200, 201, 202) {
            $row | Add-Member -NotePropertyName "ASR_MIGRATION_STATUS" -NotePropertyValue "migration started" -Force
        }
        else {
            $row | Add-Member -NotePropertyName "ASR_MIGRATION_STATUS" -NotePropertyValue "migration failed" -Force
            Write-Host "Planned Failover failed for $($row.TARGET_MACHINE_NAME). Response:`n$($res.Content)" -ForegroundColor Red
        }
    }
...
# Run planned failover (migration)
plannedfailover -CsvPath $CsvPath
