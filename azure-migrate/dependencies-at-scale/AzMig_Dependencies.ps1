...
function Get-AzMigDiscoveredVMwareVMs {
    ...
    foreach ($item in $vmwareappliancemap.GetEnumerator()) {
        ...
        |extend DependencyErrors = strcat('DependencyScopeStatus:', todynamic(properties_dependencyMapDiscovery).discoveryScopeStatus, ' Errors:', Error),OperatingSystem = todynamic(properties_guestOSDetails),Tags = todynamic(tags)
